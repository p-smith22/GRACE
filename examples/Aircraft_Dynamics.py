# ============================================================================
# aircraft_dynamics.py -- shared 6DOF aircraft model for the GRACE examples
# ============================================================================
# A mid-fidelity stability-derivative 6DOF aircraft (12 states, 3 controls) used
# as the hard, high-dimensional test case for the GRACE examples. Two entry points:
#   f(x, u)          -- plain dynamics, for shooting / obstacle avoidance
#   f_codesign(x,u,p)-- dihedral effect Clb replaced by the design parameter p,
#                       so codesign can optimize the wing dihedral (via Clb)
# State  x = [u,v,w, p,q,r, phi,theta,psi, X,Y,Z]   (body vel, body rates, Euler, position)
# Control u = [da, de, dr]  aileron / elevator / rudder deflections in DEGREES
# ============================================================================

import numpy as np
import casadi as ca

# --- aircraft constants (small UAV) ---
...
CLB0, CLP, CLDA = -0.05, -0.731, 0.075          # CLB0 = baseline dihedral-effect derivative
CNB, CNR, CNDR = 0.08, -0.060, 0.10

# --- trim state: lift = weight at V=15 m/s, alpha = 3.5 deg ---
...
# Codesign dynamics f(x, u, p): the design parameter p is the dihedral-effect Clb.
# Wing dihedral Gamma maps to Clb via Clb = Clb_base - k_dih * Gamma, so optimizing
# p directly optimizes the dihedral. (See aircraft_codesign example for the map.)
