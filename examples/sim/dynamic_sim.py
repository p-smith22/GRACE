# ============================================================================
# thruster_sim.py -- real-time interactive demo of GRACE
# ============================================================================
# A thrust-vectored vehicle (double integrator with drag) cruises to the goals you
# click, and re-plans around obstacles the moment you drop one in its path.
#
#   * left click in the arena    -> queue a waypoint (click several to build a route)
#   * drag from the left palette -> drop an obstacle into the arena
#   * drag an existing obstacle  -> move it (drop it on the palette to delete)
#   * right click                -> remove an obstacle, or unqueue a waypoint
#   * c clear obstacles, r reset, esc quit
#
# Architecture (the same split a real autonomy stack uses):
#   * MISSION PLANNER (GRACE) -- one minimum-effort solve per waypoint, run on a
#     worker thread.  Each queued segment is planned from the previous segment's
#     terminal state, so a multi-waypoint route is stitched together seamlessly.
#   * CONTROLLER (LQR) -- runs every tick, tracking the active segment.
# Dropping an obstacle re-plans in the background; the vehicle keeps flying its
# current trajectory until the new one is ready, so it never stops mid-air.
#
# The vehicle has no heading state -- thrust acts directly on velocity -- so the
# trajectories are smooth and the solves are fast (~0.02 s clear, ~1.5 s around an
# obstacle), which is what makes the avoidance feel immediate.
# ============================================================================
...
class Planner:
    ...
    def __init__(self):
        self.system = grace.build(thruster, nx=4, nu=2, N=N_HORIZON,
