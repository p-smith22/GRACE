...
eps = 0.02
phi_p  = lambda v: v + eps*v**3
phi_pp = lambda v: 1.0 + 3*eps*v**2

def f(U):
    v = D @ np.asarray(U).ravel()
    return float(np.sum(0.5*v**2 + 0.25*eps*v**4))
...
def hess(U):
    v = D @ np.asarray(U).ravel()
    return D.T @ (phi_pp(v)[:, None] * D)

def dinv(w):
    w = np.asarray(w).ravel(); U = np.zeros(n)
    for _ in range(80):
        r = grad(U) - w
...
def dprime(w):
    return np.linalg.inv(hess(dinv(w)) + 1e-9*np.eye(n))

cost = (f, dinv, dprime, grad)

H0 = hess(np.zeros(n))
band = np.abs(H0 - np.diag(np.diag(H0)) - np.diag(np.diag(H0,1),1) - np.diag(np.diag(H0,-1),-1)).max()
print("hess bandwidth-1 residual  %.1e" % band)
...
v = ca.mtimes(ca.DM(D), Uv)
nlp = {"x": Uv, "f": ca.sum1(0.5*v**2 + 0.25*eps*v**4), "g": zc - ca.DM(target)}
...
fig.suptitle("Swing-up, control-rate cost  ($N=%d$)\n"
             r"cost gap %.1e   $\|\Delta U\|/\|U\|$ %.1e" % (N, gap, ctrl_err), fontsize=10)
fig.tight_layout(rect=[0,0,1,0.94])
fig.savefig("figures/tests/Time_Derivative.png", dpi=160)
print("saved figure")